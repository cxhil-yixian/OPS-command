...
- **`DISK_DIR` / `DISK_SIZE_MB` / `MON_SEC` 選單不問**，要調就在執行 `ops.sh` 之前設成環境變數，會原封不動被子腳本繼承。
...
所以每次執行都會先取 5 秒的閒置樣本（`DUR` 比 5 小的話取 `DUR` 秒），記錄 steal、loadavg、可用記憶體、SwapFree 與換出速率，寫成報告的 `0/5` 那一節，摘要也會列成「基準」那一行。後面每一項的摘要都拿它當對照：

```
  基準   steal 0.10%，load 0.73，可用 22355MB，換出 0 KB/s
