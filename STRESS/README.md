...
- **`DISK_DIR` / `DISK_SIZE_MB` 選單不問**，要調就在執行 `ops.sh` 之前設成環境變數，會原封不動被子腳本繼承。
...
| 輪次 | fio 設定 | 量的是什麼 |
|---|---|---|
| 循序寫入 | `write` bs=1M iodepth=32 | 大檔寫入吞吐 |
| 隨機寫入 | `randwrite` bs=4k iodepth=32 | 小 IO 寫入 IOPS |
| 循序讀取 | `read` bs=1M iodepth=32 | 大檔讀取吞吐（VM 內多半不可信，見下） |
| 隨機讀取 | `randread` bs=4k iodepth=32 | 小 IO 讀取 IOPS（同上） |
...
**最後那輪「同步延遲」是新加的**：前四輪的 `iodepth=32` 量的是「佇列排滿時的吞吐」，單一 IO 的延遲被藏在佇列後面。資料庫 commit、`fsync`、寫 WAL/binlog 感受到的是「發一個 IO、等它落地」的時間，那正是這一輪在量的東西。同一顆盤的吞吐好看、同步延遲很爛，是共享雲端儲存最常見的樣子。
...
- 測試檔小於 RAM 時，提醒讀取數據會被 page cache 汙染，並算好「要開多大」的 `DISK_SIZE_MB` 給你
