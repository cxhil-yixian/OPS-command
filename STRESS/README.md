...
配置量除以約 2GB/s 就是「至少要多久才碰得完一輪」，`DUR` 比它小會先警告——碰不完一輪時 `bogo ops` 會是 0，那不是故障，是白跑（實測 25578MB 花 11.9 秒，約 2.1GB/s）。

**測試期間如果吃到 swap 會另外講。** 這一項的目標是「吃記憶體」而不是「逼出換頁」，真的換到 swap 就表示 `RAM_PCT` 對這台太貪心，量到的 `bogo ops` 裡混了換頁的代價——摘要會補上吃掉多少 swap 並建議調低比例（實測一台 RAM 3.7GB 的 VM，80% 就已經開始換頁）。
...
- **頻寬超過 2GB/s** —— 判定為在量 hypervisor 的 cache 而非實體磁碟，該數據無效，摘要那一列會標 `!! 無效 (host cache)`，並直接給你該用多大的 `DISK_SIZE_MB`。這是「有證據」的判定，一律進警告區。

  > **測試檔大過 guest 的 RAM 是不夠的**——要壓過的是 host 那層 cache，而 host 的記憶體通常大得多。實測一台 RAM 3.7GB 的 VM 用 4096MB 測試檔（已經大過 guest RAM），循序讀還是跑出 5813MiB/s。
...
**這兩件事都只在虛擬機上檢查。** 腳本開頭用 `systemd-detect-virt` 判斷，實體機的
`direct=1` 是真的直達裝置，檔案多大跟讀取可不可信無關，那條警告就不會出現；
頻寬超過 2GB/s 也不會判成「無效」（NVMe / RAID 卡本來就跑得到），只會提醒你確認
是不是量到控制器的快取。摘要的判讀提示也會跟著換成實體機的版本。
...
**`DUR` 太短的話這一項會白跑**：換頁不是一開始就有，要先把 RAM 吃滿，之後配置的部分才會被換出去。腳本會以約 2GB/s 粗估填滿 RAM 要多久，`DUR` 不到它的兩倍就先警告並建議一個值——實測一台 RAM 32GB / swap 50GB 的機器，`DUR=10` 跑完 `SwapFree` 一格都沒動。
...
**chronyd 原本是什麼狀態，測完就是什麼狀態。** 本來沒在跑的機器不會被順手打開——這件事看起來貼心，實際上很危險：實測一台 chronyd 停用、時鐘快 8 小時的 VM，測試結束時啟動 chronyd，`makestep` 直接把時間跳了 8 小時。壓力測試不該有這種副作用。所以：

| 測試前 chronyd | 測試中 | 測試後 |
|---|---|---|
| `active` | 停掉它再撥時鐘 | 啟動 + `makestep` 修殘差 |
| `inactive` | 不用停，直接撥 | **維持停用**，時鐘由 `date -s` 確定性扣回 |

chronyd 沒在跑時會另外警告一句「這台的時鐘沒有人在校正」——那通常是個該處理的問題，只是不該由壓測順手替你決定。

另外，`chronyc makestep` 回的 `200 OK` **只代表指令收到了**，真正的跳躍要等 chronyd 拿到有效測量才發生。偏差很大時會在報告印完之後才跳，所以摘要那行的「現在 XX:XX:XX」不保證是最終時間——報告裡有一行註記講這件事。

`all` 不包含此項目，需要時請單獨執行。

> 這一項是**壓力測試**：撥快時鐘是為了觀察應用的反應，測完一定還原。要**真的**改系統
> 時間或時區（機器擺錯時區、時鐘真的錯了、要開關自動校時），請用
> [`../TIME/time-set.sh`](../TIME/README.md)——那支會先算出差多少、往哪個方向、會弄壞
> 什麼再問你，而且不會順手動到 chronyd 的狀態。兩支對 chronyd 的態度是一致的。
