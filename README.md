...
選單開出來之前會先做兩件事：遠端模式下**自動更新工具腳本**（換埠進行中會跳過，
因為覆蓋 `ssh-port.sh` 會影響看門狗），以及**開場自檢** —— 必要套件缺失、fail2ban
的封鎖後端不可用 / 沒有 jail / `banaction` 對不上，都會在這時先講，沒問題就完全安靜
直接進選單。不想每次更新就加 `--no-update` 或設 `OPS_NO_UPDATE=1`。
...
- **壓力測試會真的把機器操到滿載，不要在正式環境跑**。`ram` 與 `swap` 都有觸發 OOM killer
